...
static thread_struct shell_thread, stat_thread, info_thread, help_thread;
static thread_struct hello_thread, hello2_thread;

static stk_t thread_stk[6][THREAD_STACK_SIZE - 1];

static void shell_thread_func(void *pdata);
static void print_statistics(void *pdata);
static void print_thread_info(void *pdata);
static void print_help_msg(void *pdata);
static void hello_world(void *pdata);

static tid_t shell_tid, stat_tid, info_tid, help_tid;
...
int main()
{
	...
	stat_tid = thread_create(&stat_thread,
			...
			1,
			NULL);
	help_tid = thread_create(&help_thread,
